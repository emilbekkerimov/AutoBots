    ...
    public static void main(String[] args) {
        ...
        for (Employee employee : employees) {
            averageSalaryByDep.put(employee.getDepartment(),
                    averageSalaryByDep.getOrDefault(employee.getDepartment(), 0.0) + employee.getSalary());
        ...
        Map<String, Double> avgSalaryByDepartment = employees.stream()
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.averagingDouble(Employee::getSalary)  // считаем среднюю зарплату
                ));

        avgSalaryByDepartment.forEach((dept, avg) ->
